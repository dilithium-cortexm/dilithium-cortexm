...
extern void asm_eight_mul_m3(int16_t *a, const int16_t *b);
...
void poly_frombytes_mul(poly *r, const unsigned char *a, const unsigned char *b)
{
  ...
  for(i=0;i<NEWHOPE_N/8;i++)
  {
    ...
    asm_eight_mul_m3(&r->coeffs[8*i], temp);
  }
}
  ...
  for(i=0;i<NEWHOPE_N;i+=64) /* generate a in blocks of 64 coefficients */
  {
    ...
    while(ctr < 64) /* Very unlikely to run more than once */
    {
      ...
      for(j=0;j<SHAKE128_RATE && ctr < 64;j+=2)
      {
        ...
        if(val < 5*NEWHOPE_Q)
        {
          c[count++] = (int16_t)(val - ((val>>13)*NEWHOPE_Q));
          ctr++;
        }

        if(count == (sizeof(c) / sizeof(c[0]))) {
          asm_eight_mul_m3(&a->coeffs[i+ctr-8], c);
          count = 0;
        }
      }
    }
...
extern void asm_mul_coeff_m3(int16_t*, const int16_t*);
...
{
  asm_mul_coeff_m3(r->coeffs, a->coeffs);
}
...
extern void asm_add_m3(int16_t *r, const int16_t* a);
...
{
  asm_add_m3(r->coeffs, a->coeffs);
}


extern int16_t gammas_bitrev_montgomery_m3[];
extern void asm_ntt_m3(int16_t*, const int16_t*);
extern void bitrev_m3(int16_t*);
/*************************************************
* Name:        poly_ntt
*
...
{
  asm_ntt_m3(r->coeffs, gammas_bitrev_montgomery_m3);
}
...
extern int16_t gammas_inv_montgomery_m3[];
extern void asm_invntt_m3(int16_t*, const int16_t*);
...
{
  asm_invntt_m3(r->coeffs, gammas_inv_montgomery_m3);
}
